...
async def toottally_rank(inter:disnake.ApplicationCommandInteraction, target_user:disnake.User):
    ...
    if userinfo["count"] == 0:
        emb = disnake.Embed(title="No profile found", description=f"Couldn't find a TootTally profile for selected user. It is likely that they haven't linked their TootTally profile to Discord yet")
        emb.color = disnake.Color.brand_red()
        await inter.edit_original_message(embed=emb)
        return
